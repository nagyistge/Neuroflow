...
namespace Neuroflow.Core.ComputationAPI
{
    public sealed class ComputationBuilder<T> where T : struct
    {
        ...
        public ComputationHandle Compile(ValueSpace<T> valueSpace, string name, object context = null, string debugOutputFileName = null)
        {
            ...
            var type = compiledTypes.GetOrCreate(className, () => CompileType(valueSpace, className, context, debugOutputFileName));

            IntPtr ptr = valueSpace.CloseInternal();

            var result = (ComputationHandle)Activator.CreateInstance(type, ptr, context);

            return result;
        }

        private Type CompileType(ValueSpace<T> valueSpace, string className, object context, string debugOutputFileName)
        {
            Console.WriteLine("CLASS: " + className);
            
            string source = GenerateClassToBuild(className, context != null);

            WriteSourceToTemp(source, debugOutputFileName);
        ...
        private static void WriteSourceToTemp(string source, string debugOutputFileName)
        {
            if (string.IsNullOrEmpty(debugOutputFileName)) return;

            var fi = new FileInfo(debugOutputFileName);
            if (!fi.Directory.Exists) fi.Directory.Create();
            
            File.WriteAllText(debugOutputFileName, source);
        }
        ...
        string GenerateClassToBuild(string className, bool hasContext)
        {
            ...
            sbCode.AppendLine("(IntPtr ptr, object context = null) : base(ptr, context) { }");
            sbCode.AppendLine();
            ...
            {
                var compiledBlocks = timeFrame.Select(b => CompileBlock(b.Block, b.Index, hasContext)).ToList();
                timeFrameMethods.AddLast(CompileTimeFrame(compiledBlocks, timeFrame.Key, hasContext));
            }
        ...
        CompiledMethod CompileTimeFrame(List<CompiledMethod> blocks, int index, bool hasContext)
        {
            ...
            else
            {
                ...
                bodyBuilder.Append(hasContext ? ("(" + TypeName + "* " + ComputationBlock.ValueMemberName + ", object context)") : ("(" + TypeName + "* " + ComputationBlock.ValueMemberName + ")"));
                ...
                {
                    call = hasContext ? (name + "(" + ComputationBlock.ValueMemberName + ", context);") : (name + "(" + ComputationBlock.ValueMemberName + ");"),
                    body = bodyBuilder.ToString()
                };
            }
        }

        CompiledMethod CompileBlock(ComputationBlock block, int index, bool hasContext)
        {
            Contract.Requires(block != null);
            Contract.Requires(index >= 0);
