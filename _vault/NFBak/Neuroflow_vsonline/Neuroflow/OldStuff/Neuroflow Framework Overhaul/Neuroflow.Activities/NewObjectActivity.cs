...
using Neuroflow.Activities.Design.ActivityDesigners.Interface;
...
namespace Neuroflow.Activities
{
    ...
    [Designer(typeof(NewObjectActivityDesigner))]
        ...
        where T : class
    {
        ...
        public InArgument<TValue> CreateTypeInArgument<TValue>(TValue defaultValue)
        {
            ...
            {
                return new InArgument<TValue>(new VisualBasicValue<TValue>(typeof(TValue).Name + '.' + defaultValue.ToString()));
            }
            else
            {
