...
namespace Neuroflow.Activities.Design.MVVM
{
    ...
    public abstract class ViewModel : Model
    {
        ...
        private void RegisterCommands()
        {
            ...
            var pis = GetOrRegisterCommandProps(GetType(), 
        ...
        private static PropertyInfo[] GetOrRegisterCommandProps(Type type, Func<PropertyInfo[]> createMethod)
        {
            PropertyInfo[] result;
            if (!commandPropReg.TryGetValue(type, out result))
            {
                result = createMethod();
                commandPropReg.Add(type, result);
            }
            return result;
        }
