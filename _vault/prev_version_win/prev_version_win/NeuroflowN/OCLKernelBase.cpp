...
unsigned OCLKernelBase::CalculateVectorSize(DeviceArrayFVecT* inputList) const
{
    ...
    {
        auto& f = (*inputList)[i];
        vectorSize = GetVectorSize(vectorSize, ctx->ToBuffer1(f()));
    }
    return vectorSize;
}

std::pair<unsigned, unsigned> OCLKernelBase::GetIOReduceSizesInput(DeviceArrayFVecT* inputList, OCLBuffer1* outputs, unsigned vectorSize) const
{
...
std::pair<unsigned, unsigned> OCLKernelBase::GetIOReduceSizesOutput(DeviceArrayVecT* inputList, OCLBuffer1* outputs, unsigned vectorSize) const
{
    auto sizes = make_pair(0U, 0U);
    for (unsigned i = 0; i < inputList->size(); i++)
    {
