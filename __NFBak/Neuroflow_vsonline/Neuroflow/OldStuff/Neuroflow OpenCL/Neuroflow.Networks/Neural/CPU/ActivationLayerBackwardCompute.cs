...
namespace Neuroflow.Networks.Neural.CPU
{
    public class ActivationLayerBackwardCompute : LayerBackwardCompute
    {
        ...
        protected internal override unsafe void SetErrors(float* valueBuffer, float[] errors)
        {
            Debug.Assert(OutputErrorBuffer.HasValue);

            var eb = OutputErrorBuffer.Value;
            int ebSize = eb.Size;
            fixed (float* pErrors = errors)
            {
                for (int i = 0; i < ebSize; i++)
                {
                    valueBuffer[eb.MinValue + i] = pErrors[i];
                }
            }
        } 
        ...
        {
            DataParallel.Do(ForwardCompute.OutputBuffer.Size, RunParallel, (ctx) =>
            {
