...
namespace nf
{
    struct cpp_compute_activation : virtual compute_activation
    {
        ...
        void compute_forward(const nf_object_ptr& context, const std::vector<mlp_forward_node>& nodes, idx_t bpttIterationsCount) override
        {
            forward.compute(context, nodes, bpttIterationsCount);
        }

        void compute_backward(const nf_object_ptr& context, const std::vector<mlp_backward_node>& nodes, idx_t bpttIterationsCount, gradient_computation_formula gcf, idx_t internalIterationCount) override
        {
            backward.compute(context, nodes, bpttIterationsCount, gcf, internalIterationCount);
        }
