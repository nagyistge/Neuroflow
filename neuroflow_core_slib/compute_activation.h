...
namespace nf
{
    struct compute_activation : virtual nf_object
    {
        ...
        virtual void compute_forward(const nf_object_ptr& context, const std::vector<mlp_forward_node>& nodes, idx_t bpttIterationsCount) = 0;
        virtual void compute_backward(const nf_object_ptr& context, const std::vector<mlp_backward_node>& nodes, idx_t bpttIterationsCount, gradient_computation_formula gcf, idx_t internalIterationCount) = 0;
