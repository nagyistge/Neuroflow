...
void rtlr::initialize(multilayer_perceptron* mlp)
{
    ...
    _mlp = mlp;
    _netValueDerivates = from(_mlp->_netValueDerivates.get_arrays()) >> to_vector();
    _pValuesPool = _mlp->context()->device_array_management()->create_pool(false);
    _uLayersCount = _mlp->_layers.size() - 1;
    _maxULayerSize = from(_mlp->_layers) >>
        where([](const row_numbered<layer_ptr>& l) { return l.row_num() != 0; }) >>
        select([](const row_numbered<layer_ptr>& l) { return l.value()->size(); }) >>
        max();
    ...
    sequence_marker seqMark)
{
    ...
    if (_steps.size() <= computationIndex)
    {
        ...
        else
        {
            ...
            const auto& inputLayerOfILayer = iLayer->get_input_layer(jLayerIndexN);
